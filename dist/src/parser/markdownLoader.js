...
export class MarkdownLoader {
    ...
    async readOrCreateProfile(profilePath, userId) {
    ...
    async writeProfile(profilePath, state) {
        const rendered = this.serializeProfile(state);
        await this.atomicWrite(profilePath, rendered);
    }
    async appendToDialectDelta(dialectPath, appendContent) {
    ...
    parseProfile(markdown, userIdFallback) {
    ...
    serializeProfile(state) {
        const interactionLines = state.interactionContext.length > 0
            ? state.interactionContext.map((entry) => `- ${entry}`).join("\n")
            : "- (none)";
        const candidateLines = state.unmappedCandidates.length > 0
    ...
    parseCandidates(section, fallbackPersona) {
        const lines = section
            .split("\n")
            .map((line) => line.trim())
    ...
    captureValue(input, regex) {
        const match = input.match(regex);
        return match?.[1]?.trim() ?? null;
    }
    captureSection(input, sectionTitle) {
        const escaped = sectionTitle.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        const expression = new RegExp(`${escaped}\\n([\\s\\S]*?)(\\n## |$)`, "m");
        const match = input.match(expression);
        return match?.[1]?.trim() ?? "";
    }
    renderDialectTemplate(personaName) {
        return [
            "# DIALECT DELTA",
            "",
            `Persona: ${personaName}`,
    ...
    async ensureReadable(filePath, required) {
        try {
